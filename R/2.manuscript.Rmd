...
# The cmip6_co2_obs data needs to be split into two categories to indiacate the period of time 
...
ggplot() + 
  #geom_line(data = cmip6_co2_obs, aes(year, value, color = "Meinshausen et al 2017"), size = 1.5) + 
  ...
  scale_color_manual(values = c("Hector v3" = OBS_SCHEME[["Hector v3"]],
                                "Meinshausen et al 2017" = "black" #, "Lan et al 2023" = "black"
                                )) + 
...
# is this how we should quantify the variabilty? 
fit <- lm(data = hadcrut_obs_data, value ~ year)
sd(fit$residuals)
...
```
```{r}
mean(hector_tas_v_co2$gmst/hector_tas_v_co2$co2 ) * 1000 * 3.67
...
mean(hector_tas_v_co2$gmst/hector_tas_v_co2$co2) * 1000
