...
plot(hector_gmst_decade)

hadcrut_obs_data_decade <- rollmean(hadcrut_obs_data$value, 10)


ggplot() + 
  geom_line(data = hector_gmst_decade, aes(year, value)) + 
  geom_line(data = hadcrut_obs_data_decade, aes(year, value, color = "d"))
...
ggplot() + 
  ...
  facet_grid(scenario~variable, labeller = labeller(variable = temp_labs), scales = "free") + 
  labs(y = expression("Temperature anomaly ("~degree~"C)"), x = "Year") + 
  scale_color_manual(values = c("Hector V3" = OBS_SCHEME[["Hector v3"]], 
                                "CMIP6 ESM" = "#5A5A5A")) + 
  theme(panel.grid.minor.x = element_blank(), 
        panel.grid.minor.y = element_blank(), 
...
data.frame(variables, inital_pools, post_spinup) %>% 
  ...
  write.csv(file.path(OUTPUT_DIR, "inital_carbon_pools.csv"), row.names = FALSE)
```
