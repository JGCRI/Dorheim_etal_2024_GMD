...
#for (i in 1:100){
  ...
  # Here is where need to enter in a random selection thing.... 
  inital_params <- list() # create an empty vector to store the randomly selected parameters in 
  for (p in names(params)) {
   inital_params[[p]] <- rnorm(n = 1, mean = params[[p]], sd = params_sd[[p]])
  }
 inital_params <- unlist(inital_params)
  
 set_params(inital_params, core)

  out <- tryCatch({
    step <- 0
    start <- Sys.time()
    result <- optim(params, run_hector,
                    gr = NULL,
                    core,
                    method = "L-BFGS-B",
                    lower = params_lower,
    ...
    names(result$par)
    as.data.frame(matrix(data = result$par, byrow = TRUE, nrow = 1, dimnames = list(NULL, names(result$par))))
    
  }, error = function(e){data.frame(ECS = NA)})
  
  out$name <- itteration
  
  write.csv(out, file = (paste0("fit_", itteration, ".csv")), row.names = FALSE)
